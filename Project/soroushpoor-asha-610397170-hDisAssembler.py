...
def sibDisassemble(code, mod, opsize, addrsize, rexx, rexb):
    ...
    if(mod == "00" and code[0:8] == '00100101'):
        ...
        base = convertReg('100', 0, addrsize)#rsp
        res = '['+base
        if(disp != ''):
            res += '+'+disp
        res+=']'
...
def parser(code):
    ...
    #!The main Part !!!!!!!!!!!!!!!!!!!!!
    if(code in opcode_const.keys()):#!1
        res = opcode_const[code]
    elif(code[0:4] == '1011'):#!2
    ...
    elif(code[0:5] in opcode_alternate.keys()):#!5
        if(log):
            print('alternate')
    ...
    elif(code[0:7] in opcode_acc.keys()):#!6
        if(log):
            print('acc')
        ...
        if(arg_opcode == '100'):
            ...
            res = operation + ' ' + \
                memoryaddr.split(',')[1]+','+convertImm(imm)
#we have a case for test and cmp that are different with d
    elif(code[0:6] == "100001"):#!13
        if(log):
            print('xchg, test case 13')
        memoryaddr, imm = registerAddress(
                code[8:], False, opsize, addrsize, rexr, rexx, rexb)
        if(d):
            res = 'xchg'+ ' '+ memoryaddr
